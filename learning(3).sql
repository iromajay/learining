...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `p1` () RETURNS INT(11) NO SQL
    DETERMINISTIC
return @p1$$
...
(21, 6, 1, 2, 13, '2020-03-07 09:36:22', 'Active');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ranking`  AS  select `user_answers`.`user_id` AS `user_id`,`user_answers`.`no_correct_ans` AS `no_correct_ans`,`user_answers`.`time_taken` AS `time_taken` from `user_answers` where (`user_answers`.`status` = 'Active') order by `user_answers`.`no_correct_ans` desc,`user_answers`.`time_taken` ;
